describe('Self-Sufficient E2E Tests with Real Backend', () => {
  let testUser: { email: string; password: string; token: string };

  // Before all tests, ensure we have a test user
  before(() => {
    // Create or get test user - this will register a new user if needed
    cy.ensureTestUser().then((user) => {
      testUser = user;
      cy.log(`Test user ready: ${user.email} (${user.exists ? 'existing' : 'newly created'})`);
    });
  });
  ...
  describe('Authentication', () => {
    ...
    it('should login with test user credentials', () => {
  ...
  describe('Test with Dynamically Created User', () => {
    let dynamicUser: { email: string; password: string; token: string | null };

    before(() => {
      // Create a new user for this test suite
      cy.createOrGetTestUser().then((user) => {
        dynamicUser = user;
        cy.log(`Dynamic user created: ${user.email}`);
      });
    });

    it('should login with dynamically created user', () => {
      ...
      loginViaUI(dynamicUser.email, dynamicUser.password);
    ...
    it('should access protected routes with dynamic user', () => {
      // Login first
      cy.session('dynamic-user', () => {
        loginViaUI(dynamicUser.email, dynamicUser.password);
      });
